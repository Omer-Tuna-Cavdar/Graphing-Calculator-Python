...
class GraphingCalculator(QMainWindow):
    ...
    def initUI(self):
        self.setWindowTitle("Math Project 2024")
        ...
        self.calculate_btn = QPushButton("Calculate Y", clicked=self.calculateY)
        ...
        table_btn = QPushButton("Table of Values", clicked=self.showTable)
        ...
        layout.addWidget(QLabel("Enter X value:"), 3, 0)
        layout.addWidget(self.x_value_input, 3, 1)
        layout.addWidget(self.calculate_btn, 3, 2)
        layout.addWidget(plot_btn, 4, 0, 1, 7)
        layout.addWidget(toolbar, 5, 0, 1, 7)
        layout.addWidget(canvas, 6, 0, 5, 7)
        layout.addWidget(table_btn, 11, 0, 1, 7)
        layout.addWidget(step_size_label, 12, 0)
        layout.addWidget(self.step_size_input, 12, 1)
        layout.addWidget(grid_style_label, 12, 2)
        layout.addWidget(self.grid_style_combo, 12, 3)
        layout.addWidget(apply_settings_btn, 12, 4, 1, 3)
    ...
    def calculateY(self):
        try:
            x_value = float(self.x_value_input.text())
            x_sym = sp.symbols('x')
            y_func = sp.lambdify(x_sym, sp.sympify(self.func_input.text()), 'numpy')
            y_value = y_func(x_value)
            QMessageBox.information(self, "Result", f"Y value is: {y_value}")
        except Exception as e:
            QMessageBox.critical(self, "Error", f"Invalid input or function: {e}")
